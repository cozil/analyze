...
rule CRxStuff_code
{
	meta:
		...
		script = "Type.am CRxStuff,int,code,0,$result"
		script = "$result = byte:[@pattern + 0x05]"
		script = "Type.am CRxStuff,int,codeEx,0,$result"
...
rule CRxStuff_s_code
{
	meta:
		...
		script = "Type.am CRxStuff,int,s_code1,0,$result"
		script = "$result = byte:[@pattern + 0x12]"
		script = "Type.am CRxStuff,int,s_code2,0,$result"
	strings:
...
rule CRxStuff_school
{
	meta:
		...
		script = "Type.am CRxStuff,int,school,0,$result"
		script = "Type.mcomment CRxStuff,school,\"物品所属派系: 0-无派系 1-正派 2-邪派\""
		script = "$result = [@pattern + 0x33]"
		script = "Type.am CRxStuff,int,local_career,0,$result"
...
rule CRxStuff_grade
{
	meta:
		script = "$result = [@pattern + 0x02]"
		script = "Type.am CRxStuff,int,grade,0,$result"
...
rule CRxStuff_job
{
	meta:
		script = "$result = [@pattern + 0x02]"
		script = "Type.am CRxStuff,char,job,0,$result"
...
rule CRxStuff_list_id
{
	meta:
		...
		script = "Type.am CRxStuff,short,list_id,0,$result"
...
rule CRxStuff_id
{
	meta:
		script = "$result = [@pattern + 0x03]"
		script = "Type.am CRxStuff,short,id,0,$result"
...
rule CRxStuff_k_open
{
	meta:
		...
		script = "Type.am CRxStuff,short,k_open,0,$result"
...
rule CRxStuff_s_binding
{
	meta:
		script = "$result = [@pattern + 0x02]"
		script = "Type.am CRxStuff,int,s_binding,0,$result"
...
rule CRxStuff_s_disable
{
	meta:
		script = "$result = [@pattern + 0x02]"
		script = "Type.am CRxStuff,int,s_disable,0,$result"
...
rule CRxStuff_ss_begin
{
	meta:
		...
		script = "Type.am CRxStuff,int,ss_running,0,$result"
...
rule CRxStuff_thl_skill
{
	meta:
		...
		script = "Type.am CRxStuff,int,thl_skill_state,0,$result"
		script = "Type.mcomment CRxStuff,thl_skill_state,\"仅用于谭花灵职业必杀技能，置0隐藏必杀闪烁图标\""
		script = "$result = [@pattern + 0x02]"
		script = "Type.am CRxStuff,int,thl_skill_elapse,0,$result"
	strings:
...
rule CRxStuff_sex
{
	meta:
		...
		script = "Type.am CRxStuff,int,sex,0,$result"
...
rule CRxStuff_count
{
	meta:
		script = "$result = [@pattern + 0x33]"
		script = "Type.am CRxStuff,int,count,0,$result"
		script = "Type.mcomment CRxStuff,count,\"物品数量\""
		script = "$result = [@pattern + 0x24]"
		script = "Type.am CRxStuff,int,count_hipart,0,$result"
	strings:
...
rule CRxStuff_stuff_type
{
	meta:
		script = "$result = [@pattern + 0x02]"
		script = "Type.am CRxStuff,int,stuff_type,0,$result"
...
rule CRxStuff_bbWearNum
{
	meta:
		...
		script = "Type.am CRxStuff,int,bbWearNum,0,$result"
		script = "Type.mcomment CRxStuff,bbWearNum,\"剩余耐久度\""
		script = "$result = [@pattern + 0x14]"
		script = "Type.am CRxStuff,int,bbWearMax,0,$result"
...
rule CRxStuff_bbflag
{
	meta:
		script = "$result = [@pattern + 0x02]"
		script = "Type.am CRxStuff,int,bbflag,0,$result"
...
rule CRxStuff_s_except
{
	meta:
		...
		script = "Type.am CRxStuff,int,s_except,0,$result"
...
rule CRxStuff_stoneType
{
	meta:
		...
		script = "Type.am CRxStuff,int,stoneType,0,$result"
...
rule CRxStuff_propValue
{
	meta:
		...
		script = "Type.am CRxStuff,int,propValue,0,$result"
...
rule CRxStuff_aditional
{
	meta:
		script = "$result = [@pattern + 0x03]"
		script = "Type.am CRxStuff,short,aditional,0,$result"
		script = "Type.mcomment CRxStuff,aditional,\"附加属性,为1表示存在\""
		script = "$result = [@pattern + 0x0d]"
		script = "Type.am CRxStuff,short,ad_type,0,$result"
...
rule CRxStuff_s_breaking
{
	meta:
		...
		script = "Type.am CRxStuff,char,s_breaking,0,$result"
...
rule CRxStuff_advflag
{
	meta:
		...
		script = "Type.am CRxStuff,char,advflag,0,$result"
		script = "Type.mcomment CRxStuff,advflag,\"奇玉石属性标记\""
		script = "$result = [@pattern + 0x53]"
		script = "Type.am CRxStuff,short,advcode,0,$result"
		script = "Type.mcomment CRxStuff,advcode,\"奇玉石代码\""
		script = "$result = [@pattern + 0x21]"
		script = "Type.am CRxStuff,short,soul,0,$result"
...
rule CRxStuff_level
{
	meta:
		script = "$result = [@pattern + 0x02]"
		script = "Type.am CRxStuff,int,level,0,$result"
...
rule CRxStuff_lock_time
{
	meta:
		script = "$result = [@pattern + 0x02]"
		script = "Type.am CRxStuff,int,lock_time,0,$result"
		script = "Type.mcomment CRxStuff,lock_time,\"锁定时间与标记\""
		script = "$result = [@pattern + 0x72]"
		script = "Type.am CRxStuff,int,locked,0,$result"
	strings:
