...
{
	meta:
		script = "log HF_LFE = 0x{@pattern+0x1D}"
	strings:
		$pattern = { 89 8E B8 03 00 00 D9 9E 8C 01 00 00 D9 86 5C 03 00 00 DC 1D [4] DF E0 F6 C4 41 75 }
	condition:
		#pattern == 1	
}
...
{
	meta:
		script = "log HF_WPN = 0x{@pattern + 0x0D}"
	strings:
...
rule HF_HPR
{
	meta:
		...
		script = "log HF_HPR = 0x{$result}"
		script = "$result = @pattern + 0x19"
		script = "log HF_HPB = 0x{$result}"
	strings:
